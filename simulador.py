...
class SimulationConfig:
    ...
    initial_capital: float = 1800000000
    ...
    net_inmo_value: float = 500000000
    new_rent_cost: float = 1500000
...
# --- 3. MOTOR V16.9 (Sovereign Engine) ---
class InstitutionalSimulator:
    def __init__(self, config, assets, withdrawals):
        ...
        self.L_mats = [
            np.linalg.cholesky(np.array([[1.0, 0.35], [0.35, 1.0]])),   # Normal
            np.linalg.cholesky(np.array([[1.0, -0.25], [-0.25, 1.0]])), # Local
            np.linalg.cholesky(np.array([[1.0, 0.75], [0.75, 1.0]]))    # Global
        ]
    ...
    def run(self):
        ...
        df = 8
        Z_raw = (np.random.normal(0, 1, (n_sims, n_steps, 2)) / 
                 np.sqrt(np.random.chisquare(df, (n_sims, n_steps, 1)) / df)) / np.sqrt(df / (df - 2))
        inf_sh = np.random.normal(self.cfg.inflation_mean * self.dt, 
                                  self.cfg.inflation_vol * np.sqrt(self.dt), (n_sims, n_steps))
        ...
        for t in range(n_steps):
            ...
            if np.any(mc):
                current_regime[np.where(mc)[0][np.random.rand(np.sum(mc)) < self.p_exit]] = 0
            ...
            if self.cfg.is_active_managed:
                p_def[current_regime > 0] = 0.85
            
            mus, sigs = self.mu_regimes[current_regime], self.sigma_regimes[current_regime]
            asset_values[alive] *= np.exp((mus[alive] - 0.5 * sigs[alive]**2) * self.dt + 
                                          sigs[alive] * np.sqrt(self.dt) * z_f[alive] * p_def[alive, None])
            
            cpi_paths[:, t+1] = cpi_paths[:, t] * (1 + (inf_sh[:, t] + (current_regime == 1) * 0.003))

            # Hitos Extraordinarios (Fin de año)
            if (t+1) % 12 == 0:
                y_at = (t+1) // 12
                for evt in self.cfg.extra_cashflows:
                    if evt.year == y_at:
                        asset_values[alive, 1] += evt.amount * cpi_paths[alive, t+1]
            ...
            for w in self.withdrawals:
                if w.from_year <= cur_y < w.to_year:
                    m_spend_base = w.amount_nominal_monthly_start * cpi_paths[:, t+1]
                    break
            
            # Venta Casa Emergencia
            trig_mask = alive & has_house & (np.sum(asset_values, 1) < m_spend_base * self.cfg.emergency_months_trigger)
            if np.any(trig_mask):
                asset_values[trig_mask, 1] += self.cfg.net_inmo_value * cpi_paths[trig_mask, t+1]
                has_house[trig_mask] = False

            # Retiro con Protección de Bucket (Prioridad RF)
            out = m_spend_base + (self.cfg.enable_prop & (~has_house)) * (self.cfg.new_rent_cost * cpi_paths[:, t+1])
            ...
            capital_paths[:, t+1] = np.sum(asset_values, 1)
            
            dead = (capital_paths[:, t+1] <= 1000) & alive
            if np.any(dead):
                is_alive[dead] = False
                ruin_indices[dead] = t+1
                capital_paths[dead, t+1:] = 0
                asset_values[dead] = 0
...
def app(default_rf=720000000, default_rv=1080000000, default_inmo_neto=500000000):
    ...
    st.info("🛡️ **Estrategia Diego:** Protección automática de Renta Variable usando cubos de Renta Fija para baches de mercado.")
    ...
    with st.sidebar:
        ...
        p_glo = SC_GLO[sel_glo]
        st.caption(f"Impacto Stress: RV {p_glo[0]*100}% | RF {p_glo[1]*100}%")
        
        st.divider()
        sel_ret = st.selectbox("Rentabilidad Normal", list(SC_RET.keys()) + ["Personalizado"], index=1)
        if sel_ret == "Personalizado":
            c_rv = st.number_input("RV %", 0.0, 25.0, 11.0)/100
            c_rf = st.number_input("RF %", 0.0, 15.0, 6.0)/100
        else:
    ...
    with tab_sim:
        ...
        with c3: st.metric("Reserva Defensa (RF)", f"${fmt(cap_val * (1-rv_slider/100))}")
        ...
        with st.expander("💸 Inyecciones o Salidas"):
            ...
            if ce4.button("Add"): 
                st.session_state.extra_events.append(ExtraCashflow(ev_y, ev_a if ev_t=="Entrada" else -ev_a, "Hito"))
        ...
        val_inmo_input = clean_input("Valor Neto Casa ($)", default_inmo_neto, "vi_inp") if enable_p else 0

        if st.button("🚀 INICIAR SIMULACIÓN", type="primary"):
            assets = [AssetBucket("RV", rv_slider/100), AssetBucket("RF", (1-rv_slider/100), True)]
            wds = [WithdrawalTramo(0, d1, r1), WithdrawalTramo(d1, d1+d2, r2), WithdrawalTramo(d1+d2, horiz, r3)]
            cfg = SimulationConfig(horizon_years=horiz, initial_capital=cap_val, n_sims=n_sims, is_active_managed=is_active, 
                                   enable_prop=enable_p, net_inmo_value=val_inmo_input, mu_normal_rv=c_rv, mu_normal_rf=c_rf, 
                                   extra_cashflows=st.session_state.extra_events, mu_global_rv=p_glo[0], 
                                   mu_global_rf=p_glo[1], corr_global=p_glo[2])
            ...
            st.markdown(f"<div style='text-align:center; padding:20px; border-radius:10px; border-left:10px solid {clr}; background:rgba(30,30,30,0.5);'><h1>Éxito: {prob:.1f}%</h1><h3>Legado Real: ${fmt(legacy)}</h3></div>", unsafe_allow_html=True)
    ...
    with tab_opt:
        ...
        if st.button("🔍 CALCULAR ÓPTIMO"):
            ...
            with st.spinner("Simulando..."):
                if "Mix" in opt_v: vals = np.linspace(0, 100, 11)
                else: 
                    b_amt = r1 if "1" in opt_v else (r2 if "2" in opt_v else r3)
                    vals = np.linspace(b_amt*0.4, b_amt*1.6, 11)
                
                for v in vals:
                    c_rv_opt = v/100 if "Mix" in opt_v else rv_slider/100
                    a_t = [AssetBucket("RV", c_rv_opt), AssetBucket("RF", (1-c_rv_opt), True)]
                    ...
                    c_t = SimulationConfig(horizon_years=horiz, initial_capital=cap_val, n_sims=400, is_active_managed=is_active, 
                                           enable_prop=enable_p, net_inmo_value=val_inmo_input, mu_normal_rv=c_rv, mu_normal_rf=c_rf, 
            ...
            best = df_o.iloc[(df_o['p']-target).abs().argsort()[:1]].iloc[0]
            st.success(f"✅ Valor ideal sugerido: **{fmt(best['v']) if 'Gasto' in opt_v else str(int(best['v']))+'%'}**")
